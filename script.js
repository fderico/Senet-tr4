...
const DOM = {
  ...
  dadoG1Overlay:   document.getElementById("dado-g1-overlay"),
  dadoG2Overlay:   document.getElementById("dado-g2-overlay"),
  // Controlli partita
  barraPartita:    document.getElementById("barra-partita"),
  // Overlay vittoria
  vittoriaOverlay: document.getElementById("vittoria-overlay"),
  vittoriaTitolo:  document.getElementById("vittoria-titolo"),
  vittoriaSotto:   document.getElementById("vittoria-sottotitolo"),
  vittoriaAttesa:  document.getElementById("vittoria-messaggio-attesa"),
  btnRivincita:    document.getElementById("btn-rivincita"),
  btnVittoriaMenu: document.getElementById("btn-vittoria-menu"),
  // Overlay conferma
  confermaOverlay: document.getElementById("conferma-overlay"),
  confermaTitolo:  document.getElementById("conferma-titolo"),
  confermaTesto:   document.getElementById("conferma-testo"),
  // Overlay scelta colore
  sceltaColoreOverlay: document.getElementById("scelta-colore-overlay"),
}

// Restituisce sempre dado2 (unico, ora overlay sulla board)
function getDado2() {
  return DOM.dado2
}
...
function apriConferma(titolo, testo, callback) {
  DOM.confermaTitolo.textContent = titolo
  DOM.confermaTesto.textContent  = testo
  _confermaCb = callback
  DOM.confermaOverlay.classList.add("aperto")
}

function confermaSi() {
  DOM.confermaOverlay.classList.remove("aperto")
...
function resetStatoGioco() {
  ...
  // Reset scelta colore
  gameState.umanoCome = 1
  const sceltaOverlay = DOM.sceltaColoreOverlay
  if (sceltaOverlay) sceltaOverlay.classList.remove("aperto")
  ...
  // Nascondi overlay dadi
  if (DOM.dadoG1Overlay) DOM.dadoG1Overlay.style.display = "none"
  if (DOM.dadoG2Overlay) DOM.dadoG2Overlay.style.display = "none"

  // Nascondi barra partita
  const barraPartita = DOM.barraPartita
...
function tornaAlMenu() {
  ...
  const overlayVittoria = DOM.vittoriaOverlay
...
function mostraBarraPartita() {
  const barra = DOM.barraPartita
  if (barra) barra.style.display = "flex"
}

function mostraControlliGioco() {
  if (DOM.dadoG1Overlay) DOM.dadoG1Overlay.style.display = "block"
  if (DOM.dadoG2Overlay) DOM.dadoG2Overlay.style.display = "block"
  // Imposta il testo iniziale ◈ (solo al primo avvio, prima del primo lancio)
  DOM.dado1.textContent = "◈"
  DOM.dado2.textContent = ""
  // Aggancia click dei dadi
  DOM.dado1.onclick = () => giocaTurno(1)
  DOM.dado2.onclick = () => giocaTurno(2)
}

// ============ AVVIA PARTITA ============

function avviaPartita(isRiavvio = false) {
  ...
  const modoTesto = gameState.modalita === "cpu"
    ? `vs Computer (${gameState.difficolta})`
    : gameState.modalita === "online"
      ? `online — Sei il Giocatore ${online.mioNumero}`
      : `vs Umano (${gameState.difficolta})`

  log(`Partita ${isRiavvio ? "riavviata" : "iniziata"} — ${modoTesto}`, "turno")
...
function apriVittoriaOverlay(vincitore) {
  const overlay = DOM.vittoriaOverlay
  const titolo  = DOM.vittoriaTitolo
  const sotto   = DOM.vittoriaSotto
  const attesa  = DOM.vittoriaAttesa
  const btnRiv  = DOM.btnRivincita
  const btnMenu = DOM.btnVittoriaMenu
...
function _chiudiTutteOpzioni() {
  document.querySelectorAll(".menu-card-opzioni").forEach(el => el.classList.remove("aperta"))
}

function scegliVsComputer(e) {
  if (e) e.stopPropagation()
  const opzioni = document.getElementById("opzioni-cpu")
  const giàAperta = opzioni.classList.contains("aperta")
  _chiudiTutteOpzioni()
  DOM.sottomenu.innerHTML = ""
  if (!giàAperta) {
    gameState.modalita = "cpu"
    opzioni.classList.add("aperta")
  }
}

function scegliVsUmano(e) {
  if (e) e.stopPropagation()
  const opzioni = document.getElementById("opzioni-umano")
  const giàAperta = opzioni.classList.contains("aperta")
  _chiudiTutteOpzioni()
  DOM.sottomenu.innerHTML = ""
  if (!giàAperta) {
...
function apriMenuOnlineInSottomenu() {
  _chiudiTutteOpzioni()
  apriMenuOnline()
}
...
function annullaOnline() {
  ...
  DOM.sottomenu.innerHTML = ""
  gameState.modalita = "umano"
  document.getElementById("opzioni-umano").classList.add("aperta")
}
...
function applicaStatoRemoto(data) {
  ...
  if (data.risultatoDado !== null) {
    const dadoDiv = data.giocatoreCorrente === 1 ? DOM.dado1 : getDado2()
...
function avviaGioco(opzione) {
  ...
  avviaPartita()
  if (gameState.modalita === "cpu") {
    apriSceltaColore()
  }
}

function apriSceltaColore() {
  const overlay = DOM.sceltaColoreOverlay
  if (overlay) overlay.classList.add("aperto")
  // Dadi disabilitati finché non si sceglie
  if (DOM.dado1) DOM.dado1.classList.add("dado-spento")
  if (DOM.dado2) DOM.dado2.classList.add("dado-spento")
}

function scegliColore(scelta) {
  // scelta: 1 = rosso (G1), 2 = blu (G2), 0 = casuale
  const overlay = DOM.sceltaColoreOverlay
  if (overlay) overlay.classList.remove("aperto")

  let umanoCome
  if (scelta === 0) {
    umanoCome = Math.random() < 0.5 ? 1 : 2
  } else {
    umanoCome = scelta
  }

  gameState.umanoCome = umanoCome

  if (umanoCome === 2) {
    // L'umano è G2: la CPU è G1 e inizia subito
    // Scambia posizioni percorso e pedine: G1 diventa CPU, G2 diventa umano
    // Implementazione: teniamo G1=CPU, G2=umano, ma il giocatore corrente parte da G1 (CPU)
    // aggiornaSegnaliTurno già gestisce cpu+g2=spento; qui G1 è CPU quindi parte IA
    aggiornaSegnaliTurno()
    eseguiTurnoIA()
  } else {
    // L'umano è G1: tutto normale, riabilita il dado
    aggiornaSegnaliTurno()
  }
}
    ...
    for (let row = 0; row < board[col].length; row++) {
      ...
      if (contenuto) {
        ...
        if (
          ...
          !(gameState.modalita === "cpu" && gameState.giocatoreCorrente === (gameState.umanoCome === 2 ? 1 : 2))
        ) {
      ...
      // Click su casella di arrivo evidenziata (vuota) → conferma diretta
      if (èEvidenziata && gameState.selezionata && (!contenuto || contenuto.pedina.giocatore !== gameState.giocatoreCorrente)) {
        const dado = gameState.risultatoDado
        const arrivoCoord = coordFisiche(gameState.giocatoreCorrente, gameState.selezionata.pos + dado)
        if (arrivoCoord && arrivoCoord[0] === col && arrivoCoord[1] === row) {
          cell.onclick = () => confermaMossa(gameState.giocatoreCorrente)
        }
      }
...
function confermaMossa(giocatorePulsante) {
  ...
  if (èSpeciale || dadoEraSeiFaRilancio) {
    ...
    aggiornaSegnaliTurno()
    if (gameState.modalita === "online") pubblicaESalta()
    else if (gameState.giocatoreCorrente === (gameState.umanoCome === 2 ? 1 : 2) && gameState.modalita === "cpu") eseguiTurnoIA()
  } else {
    if (gameState.modalita === "online") {
...
function giocaTurno(giocatorePulsante) {
  ...
  // Al primo lancio di G2, mostra ◈ anche sul suo dado (era vuoto all'inizio)
  const dadoG2 = getDado2()
  if (dadoG2.textContent === "") dadoG2.textContent = "◈"
...
function cambiaTurno() {
  ...
  if (gameState.giocatoreCorrente === (gameState.umanoCome === 2 ? 1 : 2) && gameState.modalita === "cpu") {
    eseguiTurnoIA()
  }
}
...
function lanciaDado() {
  ...
  const dadoDiv = gameState.giocatoreCorrente === 1 ? DOM.dado1 : getDado2()
  dadoDiv.classList.remove("shake")
  void dadoDiv.offsetWidth
  dadoDiv.classList.add("shake")
...
function aggiornaSegnaliTurno() {
  ...
  const modo  = gameState.modalita
  // umanoCome: 1 o 2 — quale giocatore è l'umano in modalità CPU (default 1)
  const umano = gameState.umanoCome ?? 1
  const cpu   = umano === 1 ? 2 : 1
  ...
  ;[d1, d2].forEach(d => {
    d.classList.remove("dado-attivo", "dado-premuto", "dado-spento")
  })

  // Vs CPU: se è il turno della CPU, entrambi spenti
  if (modo === "cpu" && g === cpu) {
    d1.classList.add("dado-spento")
    d2.classList.add("dado-spento")
    return
  }

  const dadoAttivo  = g === 1 ? d1 : d2
  const dadoPassivo = g === 1 ? d2 : d1

  // Online: il dado dell'avversario è sempre spento
  if (modo === "online" && g !== online.mioNumero) {
    d1.classList.add("dado-spento")
    d2.classList.add("dado-spento")
    return
  }

  dadoPassivo.classList.add("dado-spento")

  if (fase === "lancio") {
    dadoAttivo.classList.add("dado-attivo")
  } else {
    dadoAttivo.classList.add("dado-premuto")
  }
}
...
function eseguiTurnoIA() {
  const cpu = gameState.umanoCome === 2 ? 1 : 2
  if (gameState.giocatoreCorrente !== cpu || gameState.modalita !== "cpu") return
  ...
  setTimeout(() => {
    ...
    log(`${nomeGiocatore(cpu)} lancia il dado → ${gameState.facceDado[risultato]} (${risultato})`, "dado")

    const mosse = mossePossibili(risultato)
    if (mosse.length === 0) {
      log(`${nomeGiocatore(cpu)} non ha mosse disponibili. Turno saltato.`, "nessuna")
      setTimeout(() => {
        gameState.risultatoDado = null
        gameState.faseTurno = "lancio"
    ...
    setTimeout(() => {
      ...
      gameState.selezionata = pedina
      disegnaBoard()
      setTimeout(() => { confermaMossa(cpu) }, 700)
    }, 600)
  }, 800)
}