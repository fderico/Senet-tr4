...
const DOM = {
  ...
  dado2cpu:        document.getElementById("dado2-cpu"),
  dadoCpuOverlay:  document.getElementById("dado-cpu-overlay"),
  // Controlli partita
  btnAzioneG1:     document.getElementById("btnAzioneG1"),
  btnAzioneG2:     document.getElementById("btnAzioneG2"),
  barraG1:         document.querySelector(".barraG1"),
  barraG2:         document.querySelector(".barraG2"),
...
// Restituisce l'elemento dado del giocatore 2 corretto per la modalità attiva
function getDado2() {
  if (gameState.modalita === "cpu") {
    return DOM.dado2cpu ?? DOM.dado2
  }
  return DOM.dado2
}
...
function resetStatoGioco() {
  ...
  const dado2cpu = DOM.dado2cpu
  if (dado2cpu) { dado2cpu.textContent = ""; dado2cpu.classList.remove("shake") }

  // Nascondi barre controlli e dado CPU overlay
  document.querySelectorAll(".barra-controlli").forEach(el => el.style.display = "none")
  const dadoCpuOverlay = DOM.dadoCpuOverlay
  if (dadoCpuOverlay) dadoCpuOverlay.style.display = "none"
...
function mostraControlliGioco() {
  const isCpu = gameState.modalita === "cpu"
  const wrapper = DOM.wrapper
  const barraG1 = DOM.barraG1
  const barraG2 = DOM.barraG2
  const dadoCpuOverlay = DOM.dadoCpuOverlay

  if (isCpu) {
    // CPU: layout colonna, barra G1 sotto al board, barra G2 nascosta, dado overlay visibile
    if (wrapper) wrapper.classList.add("modalita-cpu")
    if (barraG1) barraG1.style.display = "flex"
    if (barraG2) barraG2.style.display = "none"
    if (dadoCpuOverlay) dadoCpuOverlay.style.display = "block"
  } else {
    // Umano/online: layout riga originale, entrambe le barre visibili, overlay nascosto
    if (wrapper) wrapper.classList.remove("modalita-cpu")
    if (barraG1) barraG1.style.display = "flex"
    if (barraG2) barraG2.style.display = "flex"
    if (dadoCpuOverlay) dadoCpuOverlay.style.display = "none"
  }
}
...
function disegnaBoard() {
  ...
  for (let col = 0; col < board.length; col++) {
    ...
    for (let row = 0; row < board[col].length; row++) {
      ...
      if (èEvidenziata && gameState.selezionata && !contenuto) {
...
function azioneTurno(giocatorePulsante) {
  if (giocatorePulsante !== gameState.giocatoreCorrente) return
  if (gameState.faseTurno === "lancio") {
    giocaTurno(giocatorePulsante)
  } else if (gameState.faseTurno === "selezione" && gameState.selezionata) {
    confermaMossa(giocatorePulsante)
  }
  // Se fase === "selezione" ma nessuna pedina selezionata: non fa nulla
}
...
function aggiornaSegnaliTurno() {
  ...
  const btnG1 = DOM.btnAzioneG1
  const btnG2 = DOM.btnAzioneG2
  ...
  // Spegni tutto
  ;[btnG1, btnG2].filter(Boolean).forEach(b => {
  ...
  const dado2cpu = DOM.dado2cpu
  if (dado2cpu) dado2cpu.classList.remove("turno-acceso")

  const btnAttivo  = g === 1 ? btnG1 : btnG2
  const btnPassivo = g === 1 ? btnG2 : btnG1
  const dadoG      = g === 1 ? dado1El : getDado2()

  // Bottone del giocatore non di turno: disabilitato, testo fisso
  if (btnPassivo) {
    btnPassivo.textContent = "Lancia dado"
    btnPassivo.disabled    = true
  }

  // Vs CPU: turno IA — tutto disabilitato, nessun effetto visivo
  if (modalita === "cpu" && g === 2) {
    if (btnG1) { btnG1.textContent = "Lancia dado"; btnG1.disabled = true }
    return
  }
  ...
  if (fase === "lancio") {
    btnAttivo.textContent = "Lancia dado"
    btnAttivo.classList.add("turno-attivo")
    btnAttivo.disabled    = false
  } else {
    dadoG.classList.add("turno-acceso")
    btnAttivo.textContent = "Conferma"
    if (gameState.selezionata) {
      btnAttivo.classList.add("turno-attivo")
      btnAttivo.disabled = false
    } else {
      btnAttivo.classList.add("turno-premuto")
